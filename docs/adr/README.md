...
| [0009](./0009-platform-audit-trail.md) | 平台 audit trail(通用 `AuditLog` 與 `RequestEvent`/`LedgerAdjustment` **共存** · 白名單 before/after · 權限矩陣 code-derive 不起 permission table · 平台 vs ServiceNow audit 分工) | **Proposed** | 2026-07-20 | Chris 提 audit 需求;**待拍板 OQ-1 記唔記 before/after · OQ-2 PII 策略**;前置分析 `02-architecture/audit-and-integration-observability.md` |
