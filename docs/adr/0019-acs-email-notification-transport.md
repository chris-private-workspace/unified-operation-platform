...
- ⚠️ **`APP_BASE_URL` 係第三個「唔畀探」嘅配置**(W41 新增,`auth.controller.ts` 用 `config.get` 而非 `getOrThrow`,理由同 D4:可選功能配置錯唔應該令平台起唔到身)。⇒ 未設嘅失敗模式係**靜默**:`POST /auth/forgot-password` 照返 204(統一回應不變 —— 唔可以因為配置錯而變成一個可以用嚟枚舉帳號嘅訊號),用戶就係等唔到信,唯一線索係 api log 一行 `logger.error`。
- 🔴 **一條同本 ADR「唔畀探」直接相關嘅 audit 邊界(2026-07-30 修正,Chris 拍板)**:呢個檢查**必須喺 `issue()` 之前**。原本擺喺之後,於是未設 `APP_BASE_URL` 時 token 已建、audit 已寫一行 **`reason:'issued'`**,而信一封都冇寄 —— 即係 audit 會用「我哋寄咗」去答「為咩我個用戶收唔到信」。**一個會講大話嘅 audit 比一個沉默嘅更差**,因為 ADR-0009 令佢成為營運唯一應該信嘅地方。⇒ 定下嘅邊界:**配置錯屬 ops(log / 監控),唔屬業務 audit trail**;未設時唔建 token、唔寫 audit、唔燒用戶個 cooldown。呢條同 D5「唔畀探」係同一家族嘅問題 —— **平台側睇落成功但實際唔通**,只不過呢次騙嘅唔係 API 回應,而係 audit 行。詳見 W41 `plan.md` §9 changelog(已由「待裁決」→ 已修,含 fails-before 實證)。
