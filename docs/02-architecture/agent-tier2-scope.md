...
## 7. 一件要先講清楚嘅前提 —— 🟢🟢 **已收(2026-08-17),原文保留做記錄**

> **本節寫嗰陣個前提今日已經唔成立**:W46 merge 咗落 `main`(PR #114,tip `45ad525`)·
> 部署 #9 / #9b 上咗 DEV · **`A1` + `B6` 都收咗** ⇒ 下面建議嘅三步全部做完。
> `B6`(= 本節指名嗰個 `T2-c`/`T2-d` 前置)喺 DEV 實測 SSE **200** `text/event-stream`。
>
> 🔴 **但呢節唔可以刪,因為個形狀會逐個 phase 重現** —— `T2-a`(W47)做完之後,同一句
> 嘢就變成「**W47 一行都未入 `main`**」,而下面「每一個 Tier 2 phase 都會由呢條 branch
> 開,branch 之間嘅距離只會越拉越遠」**換個 branch 名照樣成立**。
> ⇒ **當佢係一條常設規矩,唔係一次性提醒:開下一個 Tier 2 phase 之前,先確認上一個
> 已經入咗 `main`** —— 而「入咗」要逐個 commit `git merge-base --is-ancestor` 驗,
> **唔係睇 PR state**(§0 先例:PR **#87** 顯示 `MERGED`,實際只入咗 6 個入面頭 2 個)。
