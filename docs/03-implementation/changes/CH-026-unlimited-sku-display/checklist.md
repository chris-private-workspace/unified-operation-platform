...
- [x] **G-7** 🟢🟢 **2026-08-13 做咗** —— 人手 curate 嗰 22 個 SKU(~~Chris 落 UI 做~~ **Chris 2026-08-13 批准由 AI 經 API 做**:①環境 = **本機**(DEV 做唔到,佢跑緊 `dev-86ed450` **冇 CH-026 code** —— row 冇 `seatModel`、stats 冇 `unlimitedSkus`)②名單 = **22 個全部標 `unlimited`**)
  ...
  - 🟢 **DEV 側同日亦 curate 咗(2026-08-13,Chris 批;部署 #6 之後 DEV 先做得到)**:pre-state 實測 `prepaid=22`(證明之前真係未做過)⇒ 22 × PATCH **22/22 全部 200 + `unlimited`**。`totalOwned` **4,240,459 → 20,459**、`unlimitedSkus` 0 → **22**、22 行 `unallocated` 全部 `null`;**三路對數**(endpoint = 自己由 prepaid row 加返 = `4,240,459 − 4,220,000`)。⚠️ **哨兵值總和 4,220,000 兩個環境一模一樣** —— 因為佢哋 sync 緊**同一個 M365 tenant**
  - 🔴 **DEV 揭到一個本機睇唔到嘅後果:`totalUnallocated` 變成負數(`−25,151`)**。核過條數係**啱**嘅:DEV `totalAllocated = 58,814` **包含 unlimited row 嘅 allocation**(約 13,204),而 `totalUnallocated` **只計 prepaid** ⇒ `20,459 − 45,610 = −25,151`。⇒ **呢個正正係本單 progress「決定 / 偏離」#4 寫低嗰個代價**(「`totalAllocated`/`totalAssigned` 唔剔 unlimited … 代價係兩個 KPI 範圍唔同 ⇒ 靠 sub-line 明文講」)**第一次喺真 allocation 數據下浮面** —— 本機 `totalAllocated = 0`,所以由頭到尾睇唔到。
    **#4 預見咗「範圍唔同」,但冇預見「會出負數」。** 🚧 **未處理,未開單** —— 要決定嘅係:①負數係咪誠實(DEV 真係 `skusOverAllocated: 68`,佢冇講錯)②定係 sub-line 應該改寫法。**同 spec §4 標低嗰個「unlimited SKU 嘅 drift 點計」一樣,屬同一批「unlimited 落地之後先睇得到」嘅問題,建議一齊開。**
