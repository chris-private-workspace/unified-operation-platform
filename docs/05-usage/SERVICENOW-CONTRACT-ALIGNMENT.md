...
- **✍️ 你答**(2026-07-29,Chris):
  - **G3** ✅ —— 「喺 UOP **只需要處理 catalog task 嘅狀態就足夠**,RITM 嘅狀態由 ServiceNow workflow 去處理」⇒ 方案 B 確立
  - **G4** ✅ —— **有寫權**,因為係 **admin 權限**嘅帳號
  - **G5** ✅(平台自行查證,唔使你答)—— `sys_choice` 即使 admin 都返 403(table-level 限制),所以由 800 筆真數據反推:`3` ×595(**全部 inactive**)· `7` ×151 · `1` ×24(**全部 active**)· `4` ×24 · `-5` ×4 · `2` ×2 ⇒ 平台 **close 寫 `3`、hold 寫 `2`**。
    ⚠️ **若呢個對應唔啱,請話我知** —— 呢個係由數據推,唔係由你哋 choice list 確認。
  - **G6** ✅(平台自行查證)—— 規則 = **該 RITM 底下唯一嗰張 `active=true` task**。772 張 RITM 抽樣:總 task 數 1/2/3 張分別 745/26/1,但 **active task 數只有 0 或 1,冇一張有 2 個或以上**。⇒ 多張 task 係順序行,唔會同時 active。
    平台仍然會寫 `≥2` 嘅 fail-closed 分支(唔郁 + 入失敗佇列等人手),因為抽樣冇出現唔代表唔會發生。
  - **G7** ✅ —— 併入 G3:`stage` 係 RITM 屬性,交返 SN workflow
  - **G8** 🔴 **仍然要** —— 請畀一張測試用 RITM(連底下 task),做 live 驗證用

### 🅖-3 順帶提出:integration 帳號權限範圍(**唔急,但建議 go-live 前處理**)

G4 嘅答案係「admin 權限帳號」。平台實際只需要:**讀** `sc_req_item` / `sc_task`,**寫**該兩張表嘅 `state` / `work_notes` / `close_notes`。

用 admin 帳號嘅代價:平台任何一個 bug 嘅爆炸半徑,由「改錯一張單」變成「admin 做得到嘅任何嘢」。

- **✍️ 可唔可以開一個 scoped 嘅 integration role 代替?**:______________________________________________
