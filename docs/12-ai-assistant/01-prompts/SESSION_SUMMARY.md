...
**當前座標(2026-08-02)**:git 連 GitHub **private**(`chris-private-workspace`,`main`)。Backend `apps/api`(NestJS)、`/docs/api` 200、DB seeded(**24** OpCos + admin + catalog SKU)。`apps/web` = **約 10 個實畫面**(Overview / SKU Catalog / Requests + detail + new[開單] / Drift / License Assets / Settings / **Audit log** / **Delivery failures** / Login)。**api ~746 test(64 suites)· web ~253 test(28 files)**。ADR 到 **0022** · CH 到 **018**。
> ⚠️ **要匯出 catalog / 填 allocation template 之前先睇呢句**:`Export CSV`(SKU Catalog,CH-018)同 `Download template`(Settings → Integrations,W35 F2)係**兩份唔同嘅檔** —— template 已經 pre-fill curated alias 同現有 allocation(拎去改數字上傳),export 係整個 active catalog 嘅唯讀參考(睇有咩 SKU / 邊啲未 curate)。兩者都**只有 active SKU**(`catalog.service.ts:112` 硬 filter)。
