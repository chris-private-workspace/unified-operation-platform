...
| 🆕 F10-2e | **`agent-run.controller.ts` / `agent-approval.controller.ts` 兩個都冇 spec 檔** | F10-2 嘅 `runState` test 落咗喺 service 層(query shape 嘅正確位置),**但 controller ↔ DTO 嗰條縫仍然冇嘢守**。而 **BUG-011 個教訓逐字就係呢條縫**:`IntegrationController.list()` 逐個欄砌回應、明文唔 spread(ADR-0013 D2 刻意設計)⇒ 加咗欄落 read-model **唔等於出到 API**,而三層 test 可以全綠(service spec 打 service · UI test 自砌 fixture · **DTO 冇宣告嗰個欄所以 tsc 唔返佢完全合法**)。當時補咗 `integration.controller.spec.ts` 守住;agent 側**未補** | **期二 G1 之前**(G1 會加 `propose_assign`,即第一個會令 DTO 同 read-model 一齊郁嘅改動) |
