...
| G8 | live 驗(本機 + DEV) | 兩個 profile 分得開 | **Yes** | ✅ **全收** | 本機 ✅ `F7-1` —— 而「分得開」唔係「兩行唔同名」:**同一段 text · 同一個 model · 唯一變數係 prompt** ⇒ 一個提 2 個 SKU、一個提 1 個。🟢🟢 **DEV ✅ 2026-08-17 做咗**(部署 #10 之後,`F7-4`)—— 同本機同一個對照設計:同一段 request text · 同一個 model `gpt-5.6-luna` · **唯一變數係 prompt**。`dev-g8-baseline`(內建 prompt)→ **2 個 SKU**(`Microsoft_365_E5_(no_Teams)` + `POWER_BI_PRO`);`dev-g8-powerbi-only`(141 字 custom prompt)→ **1 個**,reasoning 明文寫住 **「ignoring the Microsoft 365 E5 request」**。⚠️ **request text 係我 PATCH 落去嘅**(DEV 原本嗰啲係 `update 1 idk` / `14142521`,分唔開嘅原因會同 prompt 無關)—— 改之前 snapshot 咗四個 patchable 欄,做完還原返 `update 1 idk`。收工:兩個 run hide 咗、兩個 profile `active=false`。🔴 **順帶第一次 live 命中 `R15`** —— 見下。以下係做之前嘅原文 ⬇️ **DEV ❌ 仍然未做**(`F7-2`)。🔴 **2026-08-17 更正一個 handoff 前提**:當時寫住「做完部署 W47 就全收」,**唔啱** —— 部署 #10 之後 DEV `/agent/profiles` 實測 **`[]`**,即係話 `G8` 要求嗰件事(開兩個只有 prompt 唔同嘅 profile,各跑一次,比對提幾多個 SKU)**根本唔係部署做得到嘅** ⇒ 佢係一個**獨立 live 測試**,而且成本唔係零(兩次真 model call + 喺 DEV 留低兩個新 run row) |
