...
> 由 `plan.md` derive。🟢🟢 **plan `status: closed`(2026-08-18)** —— 開工時 `active`
> (Chris 2026-08-17 approve,四條 OQ 全答),acceptance **8/8** 收齊之後轉 `closed`。
...
- [x] F7-5 ✅ **答案係「做」,而且同日做咗**(見 `F7-4`)—— 呢條**刻意保留唔刪**,因為佢記錄住一個真實嘅決策時刻:嗰一刻「未做」唔係漏做,係一個**有成本、要人拍板**嘅狀態(兩次真 model call + 喺 DEV 留低兩個新 run row)。📌 **而個成本後來兩樣都處理咗** —— run 用 CH-031 hide 走、profile `active=false`,即係話**當時嗰個顧慮係啱嘅,只不過平台啱好啱前一日先有工具解決佢**。以下係原文 ——
  - 🚧 ~~**原本嗰條(保留做記錄)**:`G8` DEV 半邊仍然未做 —— 要開兩個 profile(同 model、唔同 prompt)各跑一次,比對提幾多個 SKU。**成本唔係零**:兩次真 model call + 喺 DEV 留低兩個新 run row(而部署 #10 啱啱先 hide 走舊嗰兩個)⇒ **要 Chris 話事**~~
