...
|---|---|---|---|---|
| **B1** | 🔴 **仍未解(2026-08-04 深化)** —— registry 知道咗係 **`acrrci3ailanding1.azurecr.io`**(cross-tenant SP `4a6e1474-…` 有 AcrPush),但**兩條 build 路實測都斷**:①`az acr build` —— SP 對該 registry **冇 management plane 存取**(`could not be found in subscription`;佢喺我哋 sub 只有 `Reader`)⇒ 開唔到 task run ②本地 `docker build`+push —— Docker Hub CDN **503**(pull 唔到 `node:20-slim`)**兼** ACR firewall **`DENIED: client with IP '165.85.7.2' is not allowed access`** | **確定** | **致命** —— 冇 image 就乜都部署唔到 | **2026-08-05 更新:卡位變咗,解法由二變三**。✅ firewall 已修好(`docker login` → `Login Succeeded`)⇒ **push 側解封**;❌ 但**兩個 SP 都冇 management plane 存取** registry(`d2f094a3` 實測同樣 `could not be found in subscription`)⇒ `az acr build` 仍然唔得;❌ registry `/v2/_catalog` 顯示 7 個 repo **全部係應用 image,冇 base image mirror**。⇒ 而家卡喺「**攞唔到 base image 嚟 build**」而唔係「入唔到 registry」。<br>**三選一**:①🥇 SP 攞 registry `read` + **`scheduleRun/action`**(後者**唔喺 `AcrPush`**)⇒ `az acr build`,base image Azure 側 pull ②infra 代 build ③🆕 infra `az acr import` 兩個 base image 入 registry,我哋加 `ARG BASE_REGISTRY`(**預設空令 UAT 逐字不變**)<br>✅ pull 側擔心已撤銷(ACR 有 private endpoint) |
...
|---|---|---|---|
| 2026-08-07 | **v1.5 — F9 改行 ADR-0028(server-side code exchange);MSAL 由 `apps/web` 移除** | B9 三樣缺失入面,②Application ID URI **三輪往返都攞唔到**(infra 先後答「web portal 網址」/「OAuth authorization endpoint」/「Application ID = client id」—— 唔係態度問題,係個詞容易被讀成「應用程式嘅網址」)。而查證揭到一件更重要嘅事:**infra 配嗰個 app,本身就係另一條路嘅完整形狀** —— client secret ✅ + redirect URI ✅ + confidential client ✅,而 MSAL SPA 要嘅三樣一樣都冇。**Chris 2026-08-07 拍板**:「跟隨番 infra 那一邊的配置去改動本項目的 SSO auth 流程,不要再浪費時間去堅持原來的設計」⇒ **ADR-0028 Accepted**(supersedes ADR-0003)。deliverable 冇變(F9-8 仍然係「SSO 通 + break-glass 通」),**手段**同**前置條件**變:F9-5b 由 blocker 變無關,F9-6 由「傳 3 個 build-arg」變「唔再需要任何 Entra build-arg」。🔴 順帶一個落 plan 時冇預見嘅收穫:**Entra 配置由 build-time 降做 runtime**,改 client id / redirect URI 唔使重 build image | Chris Lai |
