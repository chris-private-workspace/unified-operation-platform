...
- [x] F9-8 🟢🟢 **兩半都收咗(2026-08-13)** verify:**SSO 登入通 + break-glass 仍然通** —— 兩邊都要驗,唔可以只驗新嗰邊。⚠️ ~~**要喺公司網做** —— build host 喺 Azure 段,`rapo-uop-web-dev.rci-t.com` → `No such host is known`(符合 B8:企業內部 DNS)~~ 🔴 **呢個前提 2026-08-13 被推翻** —— **本台機直接打得通**(Step 0 四個 endpoint 全部真答),`B8` 已解封
  - 🟢 **break-glass 嗰半 ✅ 收** —— 見 `F6-6`:login **200** + `Set-Cookie: uop_access, uop_refresh` + `GET /api/me` **200** role **`ADMIN`**
  - 🟢🟢 **SSO 嗰半 ✅ 收 —— Chris 2026-08-13 本人測試,確認可以登入。**
    🔴 **證據來源同上面嗰半唔同,要標明清楚**:break-glass 嗰半係 **AI 用 tool 驗**(`F6-6`,有 HTTP 狀態同 `/api/me` body);**SSO 嗰半係 Chris 人手驗**(Entra 互動頁要真人帳號 + MFA,AI 結構上做唔到)。⇒ **兩者都算數,但唔可以寫成同一種證據** —— 沿用 `CH-015` 個先例(「前端嗰條 acceptance 係 Chris 人手驗嘅,唔係 AI」)。
    **前置全部一早就緒**(所以先至係「差一個人」而唔係「差嘢做」):`/api/auth/sso/status` → `{"enabled":true}` · 四個 `ENTRA_*` env 由 `F9-7` 落齊(**一早做咗,只係冇勾**)· ADR-0028 server-side code exchange 令 SSO 同 break-glass 由 `grantSession` 之後**完全一樣**。
    ⇒ **`AUTH-2b`(掛咗一個月)同時解封** —— 佢由頭到尾就係等呢一撳。
