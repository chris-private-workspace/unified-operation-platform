...
| FE-Assets | **前端 License Assets 畫面**（owned/allocated/assigned ledger 數量表 + utilization bar + Headroom/Over-allocated + Manage）—— FE-1 deviation 移出 | 候選（**前置 BE-ledger-read ✅ 已完 → 解封**） | 一個前端 phase 砌,consume `GET /license/ledger`（rows + headroom/overAllocated）+ utilization bar,對 prototype License Assets view;真 allocated 生產數需 deploy curation | `design_handoff .../full-console.html`（License Assets view） · `W14-ledger-read/` |
