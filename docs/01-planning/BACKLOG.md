...
| AUTH-4c-B | **Session hardening**（refresh token + httpOnly cookie:localStorage→cookie + rotation + guard 讀 cookie + CSRF SameSite + POST /auth/refresh·logout） | 候選 **W21**（前置 4c-A;**ADR-0006 §7** 架構改,單獨 phase 隔離風險） | 一個 phase：改 working session model（guard/api.ts/CSRF）+ RefreshToken 表 | ADR-0006 §7 · `W20-*/plan.md §1(Out)` |
