...
| BE-tenant-owned | **後端 tenant-owned 層 endpoint**（pull subscribedSkus `prepaidUnits.enabled` = M365 owned/prepaid 總數,per-SKU）—— 解封 Assets **Platform mode**（Owned/Unalloc 三層 + recon tile） | 候選（W15 honest gap carry-over） | 一個後端 mini-phase（`src/integration/graph` getSubscribedSkus 已有,加 owned 總數 read）+ 隨後 FE Assets Platform mode tab（Owned/Allocated/Assigned/Unalloc 欄 + 三層 recon strip） | `W15-fe-assets/plan.md §1（Out）` · `design_handoff .../full-console.html`（Platform mode） |
