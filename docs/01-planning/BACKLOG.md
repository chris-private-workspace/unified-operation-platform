...
| AUTH-3b | **前端真 role scope**（consume /me 真 role/opcoScope + 移除假 `role` toggle + "My queue" 解封 + Overview/Requests 隨 role scope + 真 OPCO_IT SSO e2e） | 候選（前端;真 e2e 卡 IT app reg — 隨 AUTH-2b。**註**:本地登入[W18]已可攞真 /me role → 假 toggle 移除唔再全卡 SSO） | consume `GET /me`（本地登入或真 SSO）+ 移除 `store/ui.ts` 假 toggle;真 OPCO_IT SSO e2e 仍隨 AUTH-2b | `W11-*/plan.md §1（Out）` · `store/ui.ts` 假 toggle |
