...
| AUTH-4c | **密碼生命週期**（自助改密碼 / admin 重設 / lockout / rate-limit / 密碼 policy;refresh token;httpOnly cookie hardening） | 候選（前置 AUTH-4a ✅） | 一個 phase；**密碼重設 via email 需 email transport 決定[SMTP/SendGrid/Graph sendMail = 另一 H2 sub-decision]** | ADR-0005 §6（4c）· `W18-auth-local-login/` |
