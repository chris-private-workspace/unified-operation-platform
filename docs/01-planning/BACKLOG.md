...
| AUTH-4b | **本地 user 管理**（admin 建/列/改/停用本地 user + 設 role/opcoScope + Settings › Users&roles UI 取代 coming-soon stub） | 候選（**前置 AUTH-4a ✅ 已完 → 解封**） | 一個 phase：admin CRUD endpoint（`@Roles(ADMIN)`）+ FE Users&roles 表接;初始密碼 admin 設 | ADR-0005 §6 · `W18-auth-local-login/` |
