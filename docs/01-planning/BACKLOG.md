...
**最後更新**:2026-07-09(**FE-2 完成** — W07 Requests 列表 + detail 讀+寫,advance/mark-synced round-trip 驗;揭出後端 crash bug BUG-002 候選;下一個 = FE-3 或 BUG-002)
...
> **開發路線（2026-07-09）= Backend-first**:W02 C ✅ → W03 D-1 ✅ → W04 D-2 ✅（後端業務層完成）→ **W05 FE-scaffold ✅ → FE-1 ✅**（W06）→ **FE-2 ✅**（W07,Requests 讀+寫）。前端進行中:**FE-3**（下一個候選,Drift/Settings/Login）→ **FE-Assets**（前置 BE-ledger-read）→ AUTH → DEPLOY。⚠️ **BUG-002**（後端 assign crash）可能插隊先修。
...
| BUG-002 | **後端 assign crash**:`findUser` throw Graph/MSAL error（非 return null）未 wrap → invalid status(-1)→ **NestJS process crash**（critical path robustness;FE-2 assign 測試揭出） | 候選（等 owner 定即修 vs defer） | `assign.service.ts` findUser 呼叫 try/catch → clean HttpException（+ H5 test 覆蓋 throw 路徑;W04 全 mock return value 漏咗） | `W07-fe-requests/progress.md`（Day 1 🔴） · `apps/api/src/fulfilment/assign.service.ts` |
