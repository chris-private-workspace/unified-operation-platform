...
| W24 | **Phase 甲 inbound intake**（n8n onboarding request push → 平台 `POST /requests/intake` + m2m auth + 即時建本地 Request mirror;ADR-0008 D2/D4/D6） | 🔵 **進行中·⏸️暫停**（2026-07-15;**D0 doc-sync ✅ · D1 合約+agenda ✅**;待對接 n8n team） | **明天**:用 `N8N-AGENDA.md` 對接 n8n team（A 組 4 流程決定[intake SSoT/誰 assign/push 唔 block/sync 時序]+ B 組 6 欄位）→ 填決定回寫 CONTRACT/ADR/DESIGN → 落 D2 code | `W24-request-intake/` · `docs/adr/0008-*` |
