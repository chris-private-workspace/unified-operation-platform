...
- [x] F2-6 ✅ **DEV migration 真 apply(部署 #11 `dev-b4915e9`,2026-08-19)** —— break-glass 登入後 `GET /api/agent/conversations` → **200 `[]`**。🔴 **`200 唔係 500` 先係佐證**:表唔存在 Prisma 會掟 `PrismaClientValidationError` ⇒ 500,而 `[]` 只係未有 row。**刻意唔睇 revision status**(見 `F7-4`)。順帶 OpenAPI 對數:73 條 path,W48 **七條全在**(六條 conversation + `/agent/profiles/options`)
  <br>🟢 **同日稍後(W49 Day 4)喺瀏覽器補多一層,而佢比 `200 []` 完整**:真開一條對話 ⇒ conversation detail **兩個 `turn`(user + assistant)** ⇒ **`AgentChatTurn` 唔止存在,仲寫得入**。📌 `200 []` 證嘅係**表讀得到**,呢個證嘅係**兩張表都寫得入** —— 一條純讀嘅 response 對一個 additive migration 嚟講係必要但唔充分。⚠️ 全程一樣冇睇 revision status
...
- [x] F7-3 ✅🟢🟢 **2026-08-19 部署 #11 之後全收**(證據喺本項尾)—— DEV:migration + 一條真對話 + **一次真 SSE 通知 + refetch**(⚠️ **唔可以引用 `B6`** —— 佢證嘅係 heartbeat + 短事件)。🔴 **本條 2026-08-19 `F8-1` 掃出寫錯咗兼更正**:原文寫「一次真 **token stream**」,而 `F4` 08-18 已經由 token-by-token 收窄做 **turn-level notify**(plan §8 有記,理由係真 token 流要擴 seam = H1,兼且 token 未經 `scrubPii` 就落 wire)⇒ **驗嘅嘢由頭到尾唔應該係 token**。📌 呢個就係 W47 教落「`F8-1` 要逐條掃 acceptance 句」嘅原因 —— plan §2 個 `F4` acceptance 其實**冇**寫 token(佢寫「DEV 真通」),stale 嘅只有 checklist 同 `G9` 兩處措辭。
  <br>🟢🟢 **收貨證據 A —— wire 側(2026-08-19)**:4 個 turn(兩問兩答)· 兩個 run 都 `completed`。**SSE 用直讀 wire 驗唔用瀏覽器**,因為 DEV 側真正未知數係「捱唔捱得過 ACA ingress + nginx」(同 `F6-14` 400 body 同族),而 **buffering 呢種失敗喺瀏覽器睇落同「agent 未答完」一模一樣**;前端 refetch 嗰半 `F5-11` 本機驗過。**連線喺送 turn 之前開好** ⇒ 收到嘅一定係 server push:`POST /turns` 201 @ `10:26:42.183` → `changed` @ **`.262`(79 ms)** → `changed` @ `10:26:44.134`(assistant turn)→ `ping` ×3 每 **25 秒**。event **逐個即時到**,唔係最後一次過吐 ⇒ **proxy 冇 buffer**。⚠️ 順帶:回應**冇** `x-accel-buffering` header ⇒ 唔 buffer **唔係靠嗰個 header 擋**,係 nginx 配置本身;將來有人改 `nginx.conf.template` 就冇咗呢道保險而**冇任何 test 會紅**
  <br>🟢🟢 **收貨證據 B —— 瀏覽器側(同日,W49 Day 4)**:A 自己寫住「前端 refetch 嗰半 `F5-11` **本機**驗過」,而呢半係喺 **DEV 真畫面**收嘅 ⇒ **兩份夾埋先蓋齊「wire 通」同「畫面郁」**。收貨標準係「**我冇撳過任何嘢**」:送出 `What licence SKUs are available in the catalogue?` 之後**冇 reload · 冇 navigate · 冇撳掣**,`Thinking…` **自己消失**,答案**自己出喺畫面**。落 API 對數:2 個 turn · run `completed` · `profileId` = `w48-deploy-check`。
  <br>🔴 **點解呢半一直做唔到,而佢唔係「未部署」**:DEV **三個 `AgentProfile` 全部 `active: false`** ⇒ `/agent/profiles/options` 返 `[]` ⇒ `/assistant` 個 `New conversation` **disabled**。⚠️ **`GET /agent/profiles` 預設都係只返 active**(`list(includeInactive = false)`),所以第一次打嗰個 `[]` **差啲令我斷錯症做「一個 profile 都冇」** —— 要 `?includeInactive=true` 先見到三個。
  <br>🟢 **順帶兩個一直未知嘅事實**:**Azure OpenAI 喺 DEV 通**(`w48-deploy-check` 2026-08-19 02:25/02:26 兩個 run 都 `completed`,啟動者係 **`admin@uop.local` break-glass** 唔係 SSO 帳號)· **deployment name = `gpt-5.6-luna`**。
  ⚠️ **做完把 profile 停返 `active: false`**,三個 flag 同做之前逐個一致;留低一條測試 conversation(`AgentConversation` 冇 `DELETE`,`ADR-0041 D7`)
- [x] F7-4 ✅ ⚠️ **唔可以睇 revision status 當證據**(entrypoint 令 migrate 失敗 NON-FATAL)—— **本次真係冇睇**:`F2-6` 用 `200 ≠ 500`、`F7-3` 用真 turn + 真 SSE event、web 側用 bundle 字串,三條全部係「只有新 code 先出到」嘅嘢。呢條唔係一件工作,係一條約束,而佢**滿足咗**
...
>
> 🟢🟢 **而收佢哋嘅唔係「真人登入」,係 **Playwright 用緊嘅 browser 本身就有 Chris 個
> session** ⇒ AI 做得到。呢個推翻咗我一直嘅假設。
...
> (⚠️ 兩者喺畫面上一模一樣)。**🔴 W49 `F4-3` 更正咗呢條嘅成因** —— 唔係「連斷 3 次」,
> 係**零 event**(proxy 揸住個 socket,`readyState` 一直 OPEN)。
