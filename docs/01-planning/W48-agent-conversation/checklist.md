...
- [ ] F5-10 🚧 **`F5-9` 加咗新 UI 元素(`Select` + purple `Badge`)⇒ light + dark 要再 render 一次** —— **兩個都係既有 primitive / 既有 tone**(DS-8 AI → purple)所以**唔觸發 H6 STOP**,但 `F5-3` 嗰次 render **蓋唔到今日棵樹**(W47 教訓:一個勾咗嘅 gate 唔等於蓋住之後入嘅 commit)。⚠️ **要再借一次 5433**(agent picker 要真 profile 資料)⇒ 要 Chris 再批停 `ai-doc-extraction-db`
