...
| 2026-08-04 | 1.3 | **R3 deviation log — 加 F8（批 C，計劃外）**：G7 實測 `sc_item_option` PATCH **403 ACL** ⇒ ADR-0025 D3 後半（gate ② 回填 `target_user`）**做唔到**。Chris 同日 Accept **ADR-0026**（拆走回填 + 改行 work note）⇒ 本 phase 多咗一個 deliverable。**G7 個驗收準則亦要改**：「`target_user` 真係由 requester 變新用戶」由「未驗」改成「**已證實做唔到**」，改為驗 work note。順帶加 `DEFERRED_REGISTER` **DD-5** + `RISK_REGISTER` **R7**。 |
