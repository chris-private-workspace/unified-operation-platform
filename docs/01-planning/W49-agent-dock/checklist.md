...
- [ ] F5-2b 🚧 **`F4` 之後個 render 半邊要再跑一次** —— `F5-2` 嗰次 render 喺 `F4` 之前,而 `F4` 加咗**成個 chat 介面**(turns · composer · disconnected banner · proposal 提示)。📌 **同一個形狀第三次**(`F5-1` · `F5-2` · 而家)—— **勾咗嘅 gate 唔蓋住之後入嘅 commit**。⚠️ 靜態嗰批(DS-1/2/6/9)已經由 `F4` 個 source scan 覆蓋(冇 `variant="primary"` · 冇 `text-accent`),要補嘅係 **light + dark 真 render**
