...
| # | Criterion | Target | Block closeout? | 狀態(2026-08-20) |
|---|---|---|---|---|
| G1 | `Drawer` owner-approved + design-system 更新 | H6 走完合法路徑 | **Yes** | ✅ `F1-1` H6 STOP → Chris 批 · `F1-2` `design-system.md §2` **七條約束**(寫「冇 `inset-0` / 冇 scrim / 冇 `aria-modal`」三樣具體嘢,唔寫「唔好 trap focus」) |
| G2 | **non-modal 係可觀察嘅** | dock 開住,底下嘅表撳得郁(唔係「睇落唔似 modal」) | **Yes** | ✅ **本機 `F2-2` + DEV `F5-4` 各真撳一次** —— DEV:dock 開住撳 Requests 第一行,URL 由 `/requests` 變 `/requests/cmswq1v10…`。⚠️ 量度只係前提:`fullScreenOverlays: 0` · `aria-modal: null` · **`mainWidth` 開前開後都係 1224**(前後對照,唔係單邊觀察) |
...
>
> 🔴 **呢個表原本冇「狀態」欄,2026-08-20 收尾先補** —— 而**冇狀態欄嘅 acceptance 表,同全部 `[ ]`
> 嘅 acceptance 表係同一個問題**:冇人講得出仲差幾多。W46 收尾撞過(21 條全 `[ ]` 而 18 條做完),
> 呢度係同族嘅另一個形狀 —— **唔係冇更新,係結構上冇地方畀你更新**。
...
| **R8** 🆕 | 🔴🔴 **`assertOpcoScope` 喺 `POST /agent/conversations` 今日係 no-op,而佢睇落好似喺度守緊** —— 三條 link 全部驗過:①controller `@Roles(ADMIN, REGIONAL)` ②`user-admin.service.ts normaliseScope` 第一句 `if (role !== OPCO_IT) return null` ⇒ **ADMIN / REGIONAL 被強制 null scope**(DB 實測 `ADMIN 1 users / 0 with_scope`)③`assertOpcoScope` 條件係 `if (user.opcoScopeId && …)` ⇒ **過得到 role guard 嘅人一定觸發唔到佢**。live:OPCO_IT 打上去收 **`403 Insufficient role`**(guard),唔係 `Out of OpCo scope` | Med | 🔴 High | ①**唔係 dead code —— 係 defence-in-depth**,`canUseAgent` 一放寬到 OPCO_IT(scope report 提過 per-agent 範圍)就即刻生效,所以**唔應該刪** ②但**唔可以再當佢係今日嘅保護**:今日真正擋嘢嘅係 **role guard + `findUnique` 個 404** ③`agent-conversation.scope.spec.ts` 頂部已經寫明呢件事 ④📌 **形狀同 `R13` 一樣**:一個睇落喺度守緊、實際上冇被觸發嘅檢查,係一個冇人會再檢視嘅安心假象 |
