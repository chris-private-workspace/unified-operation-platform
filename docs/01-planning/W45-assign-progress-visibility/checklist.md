...
- [ ] F4-4 🔴 **live 驗**:DEV 真撳一次失敗 + 一次成功(G11)—— SKU 用 `POWERAUTOMATE_ATTENDED_RPA`(W43 查證,要先加 `allocated`)
  ...
  - [ ] **F4-4b 真撳** 🔴 **卡 `B8`** —— private DNS 完全冇配,build host 喺 Azure 段解析唔到 `rapo-uop-web-dev.rci-t.com` ⇒ **一定要喺公司網做**。💡 繞路(**未驗證**):公司網嗰台機 hosts 加 `10.160.71.70  rapo-uop-web-dev.rci-t.com`,**用 custom domain 唔好用 ACA FQDN**(infra 綁咗 SNI cert 喺 custom domain,`ENTRA_REDIRECT_URI` 亦係佢)
