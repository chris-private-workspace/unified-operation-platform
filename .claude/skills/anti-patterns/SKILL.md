...
| AP-14 | **build 綠 ≠ artifact 起得身**(gate 全部喺睇 source) | BUG-008(Sev2):CH-011 加 `scripts/*.ts` → tsc 冇 `rootDir`,emit 根由 `src/` 抬到 `apps/api/` → `main.js` 靜靜搬去 `dist/src/main.js`,而 entrypoint 跑 `node dist/main` ⇒ **每個容器一起身即 CrashLoopBackOff**。626 test / lint / `nest build` / `az acr build` **五道全綠**,冇一道問過「build 出嚟嗰嚿嘢行唔行」<br>🔴 子型 —— **build 靜靜 skip**:`tsc --noEmit` 唔建 outDir,把 incremental 狀態寫去 `dist/` **外面**(`apps/api/tsconfig.build.tsbuildinfo`),之後每次 `nest build` 都當「已是最新」→ **exit 0、零輸出、零檔案**,清 `dist/` 清唔到佢<br>**同 AP-1 分野**:AP-1 係「冇行 user flow」;AP-14 係**連 artifact 存唔存在都冇人驗**,而 test 天生見唔到(ts-jest 跑 `src/**`) | ① 改到 build config(`tsconfig*` / `nest-cli.json` / Dockerfile / 新增 `src/` 外嘅 `.ts`)嗎?→ **實際 `Test-Path dist/main.js`**,唔好信 build 綠<br>② 部署類改動有冇一道 gate 係驗 **artifact** 而唔係 source?(Dockerfile `RUN test -f dist/main.js` 就係)<br>③ build 完「零輸出零檔案」→ 先搵 stray `*.tsbuildinfo`,唔好即刻懷疑 fix 方向<br>🔴 ③b **清完有次序,唔跟就會覺得「清咗都冇用」**:刪 `apps/api/*.tsbuildinfo` **同埋** `dist/` 之後,**直接起 stack —— 中間唔可以插一次 `npm run build`**。因為 `nest start --watch` 一起身會**清 `dist/` 但唔刪 tsbuildinfo**,所以你插嗰次 build 會即刻重新生成 cache,watch 跟住清走 dist 而 tsc 判「已最新」skip emit ⇒ 原樣重現。W41 closeout **實測撞三次**先睇穿,兩次都係死喺「刪 cache → build → 起」呢個直覺次序<br>⚠️ 症狀識別:`nest start --watch` 印 **`Found 0 errors`** 而同時 stderr 報 `MODULE_NOT_FOUND` —— 兩句夾埋出現就係佢,唔使再懷疑 TS 錯誤 |
| AP-15 | **開咗 PR 之後仲繼續 push 落同一條 branch** | **累計四次**:PR **#87**(實測只 merge 咗 6 個入面**頭 2 個**,靠 checkout 之後見到舊版 working tree 先揭穿)· **#102** · **#104** · **#105**<br>🔴 **2026-08-15 搵到根因,唔再係「玄學」**:commit **到咗 remote branch**,但 **PR 個 head ref 冇跟住更新** ⇒ merge 嗰陣 GitHub 只 merge **PR 自己認住嗰個 head**,後面 push 上去嗰啲原地不動<br>🔴 **順帶推翻一個舊誤判**:`gh pr view` 報嘅 commit 數**係真嘅,唔係 stale API**。曾經當佢 stale ⇒ 結論變成「顯示問題,唔使理」⇒ **繼續 push 落同一條 PR**,即係繼續踩<br>⚠️ **2026-08-15 補一個界線,免得本條變成「查一次就落結論」** —— 同日喺 PR **#109** 實測:**push 完即刻** `gh pr view` 返嘅係**舊** head + `CONFLICTING`,而**第二次查就啱**(新 head + `MERGEABLE`)⇒ **head ref 唔係唔更新,係唔即時更新**。呢個**唔推翻上面四次**(嗰四次係 merge 完先發現內容走漏,唔係 push 後幾秒嘅事),但**「查一次見到舊 head」唔足以斷定係走漏** —— 分唔清就會把一個 stale window 當成 #87 嗰種真走漏,然後為咗一個唔存在嘅問題重開 PR<br>**同 AP-13 分野**:AP-13 係兩處各自維護同一件事實;呢條係**同一件事實嘅兩個副本**(remote branch head vs PR head ref)**由第三方系統負責同步,而佢唔保證做到** | ① **PR 一開就收口** —— 之後要補嘢,**開新 PR**,唔好 push 落已開嗰條<br>② merge 之後**逐個** `git merge-base --is-ancestor <sha> origin/main` 驗(PR #88 六個就係咁樣查過,全部 `True`)<br>③ 見到「PR `MERGED` 但 code 唔喺 main」→ **先數 PR 認住幾多個 commit**,唔好即刻懷疑 merge 策略<br>④ **查 PR head 唔可以只查一次** —— push 之後有 stale window;**兩次都仲係舊 head** 先至係 ①② 要出場嘅訊號 |
| AP-16 | **用錯工具判斷「呢條 branch 刪唔刪得」** | `git diff --stat main <branch>` 睇行數多寡,或者只信 `git branch --merged`<br>🔴 **兩個都答緊另一條問題**:`git diff` **係對稱嘅** —— 行數大細同「有冇未 merge 內容」冇必然關係(main 行先咗一大截,個 diff 一樣會好大);`--merged` 靠 **commit 可達性**,所以對 **cherry-pick / rebase 過**嘅 branch 會漏報(內容明明入咗 main,但 sha 唔同 ⇒ 佢照話「未 merged」)<br>**同 AP-15 分野**:AP-15 問「PR 認住嗰個 head 啱唔啱」;呢條問「呢啲 commit 嘅**內容**到底入咗 upstream 未」 | 用 **`git cherry origin/main <branch>`** —— **`-` 前綴 = 內容已經喺 upstream**(佢比 patch-id,唔比 sha,所以 cherry-pick 過都認得);**全部 `-` 先可以刪**,見到 `+` 就係真係有嘢未入<br>⚠️ 2026-08-12 一次過清 18 條 remote branch 就係靠呢個判準,唔係靠 `--merged` |
| … | {加你項目踩到嘅坑} | {實證} | {檢查} |
