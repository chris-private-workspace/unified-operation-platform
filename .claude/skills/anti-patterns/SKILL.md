...
| AP-15 | **開咗 PR 之後仲繼續 push 落同一條 branch** | **累計四次**:PR **#87**(實測只 merge 咗 6 個入面**頭 2 個**,靠 checkout 之後見到舊版 working tree 先揭穿)· **#102** · **#104** · **#105**<br>🔴 **2026-08-15 搵到根因,唔再係「玄學」**:commit **到咗 remote branch**,但 **PR 個 head ref 冇跟住更新** ⇒ merge 嗰陣 GitHub 只 merge **PR 自己認住嗰個 head**,後面 push 上去嗰啲原地不動<br>🔴 **順帶推翻一個舊誤判**:`gh pr view` 報嘅 commit 數**係真嘅,唔係 stale API**。曾經當佢 stale ⇒ 結論變成「顯示問題,唔使理」⇒ **繼續 push 落同一條 PR**,即係繼續踩<br>**同 AP-13 分野**:AP-13 係兩處各自維護同一件事實;呢條係**同一件事實嘅兩個副本**(remote branch head vs PR head ref)**由第三方系統負責同步,而佢唔保證做到** | ① **PR 一開就收口** —— 之後要補嘢,**開新 PR**,唔好 push 落已開嗰條<br>② merge 之後**逐個** `git merge-base --is-ancestor <sha> origin/main` 驗(PR #88 六個就係咁樣查過,全部 `True`)<br>③ 見到「PR `MERGED` 但 code 唔喺 main」→ **先數 PR 認住幾多個 commit**,唔好即刻懷疑 merge 策略 |
