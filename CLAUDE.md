...
- **仍未做 / pending**:見 `BACKLOG.md`(🔴 **AUTH-2b 真 SSO e2e = W44 `F9-8` 嗰一撳**,唔再卡 IT app registration[ADR-0028 之後現有 app reg 直接用得],**卡緊 Chris 本人做一次 Entra 互動登入**;DEPLOY-harden;W43 遺留 = live close 未驗;~~🆕 🔴 DEV 落後 `main` 五個 CH ⇒ 要部署 #6~~ 🟢 **2026-08-13 部署 #6 `dev-53965f3` 做咗,DEV 已追齊**;🚧 **但 DEV 側 `G-7` 未做** —— curate 係 DB 資料唔係 code)。🟢 **CH-026 `G-7` 2026-08-13 做咗**(22 個 SKU curate 做 `unlimited`,`Available seats` `4,270,779` → `50,779`)。
