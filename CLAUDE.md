...
- 🟢 **B1(image build)2026-08-05 已解封** —— 靠**換一台唔喺公司網嘅 build host**(出口 IP `52.187.129.166`,Azure 段;之前四條解法全部 assume 咗「build 一定要喺公司網嗰台機做」)。Docker Hub ✅ · ACR firewall ✅ · 兩個 image build ✅ · **push 真證到**(api `sha256:5a8d48cd…` / web `sha256:1d543670…`,之前只證到 `login`)。⚠️ **唔係長期方案** —— 佢繞開公司 proxy,唔係令部署鏈喺公司網跑得到;**解法 ①(SP 攞 `scheduleRun/action`)仍然係最乾淨,infra 唔應該撤走**。🔴 `AcrPush` **唔包** `scheduleRun/action` 呢個誤解值得記住。詳見 `docs/13-deployment/09-dev-as-built.md`。
