...
- **仍未做 / pending**:見 `BACKLOG.md`(✅ **AUTH-2b 2026-08-13 closed**(Chris 本人測 SSO 確認可以 ⇒ W44 `F9-8` 全收);DEPLOY-harden;W43 遺留 = live close 未驗;~~🆕 🔴 DEV 落後 `main` 五個 CH ⇒ 要部署 #6~~ 🟢 **2026-08-13 部署 #6 `dev-53965f3` 做咗,DEV 已追齊**;🚧 **但 DEV 側 `G-7` 未做** —— curate 係 DB 資料唔係 code)。🟢 **CH-026 `G-7` 2026-08-13 做咗**(22 個 SKU curate 做 `unlimited`,`Available seats` `4,270,779` → `50,779`;本機 + DEV 兩邊各做一次)。🆕 🔴 **CH-029 開咗但係 `proposed` —— 三條 OQ 未答,未 approve,零 code,唔好直接開工**:三個缺口(**A** 重複 assign 令 ledger double-count[**W39 OQ-1 拍板嘅結果,唔係 bug**]· **B** `totalUnallocated` 喺真 allocation 數據下變負數 · **C** drift 對 unlimited SKU 冇意義但照開 alert)。**A 同 C 觸發 H1**(ledger 兩層數字 + 對帳方案甲都係 §5.1 lock 決策;A 仲要推翻 W39 OQ-1 兼軟化 ADR-0017 D0)⇒ **approve + ADR 之後先開工**。
