...
- **仍未做 / pending**:見 `BACKLOG.md`(✅ **AUTH-2b 2026-08-13 closed**(Chris 本人測 SSO 確認可以 ⇒ W44 `F9-8` 全收);DEPLOY-harden;W43 遺留 = live close 未驗;~~🆕 🔴 DEV 落後 `main` 五個 CH ⇒ 要部署 #6~~ 🟢 **2026-08-13 部署 #6 `dev-53965f3` 做咗,DEV 已追齊**;🚧 **但 DEV 側 `G-7` 未做** —— curate 係 DB 資料唔係 code)。🟢 **CH-026 `G-7` 2026-08-13 做咗**(22 個 SKU curate 做 `unlimited`,`Available seats` `4,270,779` → `50,779`;本機 + DEV 兩邊各做一次)。🆕 **CH-029 開咗,三條 OQ 同日答晒,但仍然 `proposed` —— `ADR-0034` 未 Accepted,零 code,唔好直接開工**。三個缺口:**A** 重複 assign 令 ledger double-count(**W39 OQ-1 拍板嘅結果,唔係 bug**;`POWERAUTOMATE_ATTENDED_RPA` 就係活例)· **B** `totalUnallocated` 負數 · **C** drift 對 unlimited 冇意義但照開 alert。**Chris 答案**:A = **assign 前由平台自己查 M365** · B = **負數係誠實,唔改** · C = **先攞實測數**。🟢🟢 **A 揀「平台自己查」令道閘企喺 provider 之前 ⇒ 兩條路自動一致 ⇒ 唔使軟化 ADR-0017 D0,反而係 D0 嘅正確應用**。🟢 **實測(DEV reconcile)**:**72 個 OPEN drift alert,16 個(22.2%)屬 unlimited** ⇒ 跳過後 72 → 56;🔴 **順帶揭到:72 個入面只有 4 個 `ledgerAssignedSum ≠ 0`** ⇒ **今日大部分 drift 唔係「拉開咗」,係「平台由頭到尾未記錄過」**(指向 ADR-0014 baseline,唔喺本單)。🔴 **下一步 = approve ADR-0034 + 揀 `D3`**(skip 之後 stage 點走;**揀 C[新 stage]就會掂 stage machine ⇒ 即刻升 Phase**)。
