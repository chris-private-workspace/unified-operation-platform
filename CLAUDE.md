...
- 🟢 **B9(SSO)—— 2026-08-07 改咗設計解封,code 齊,但仍未 live 驗過**。而家行緊 **break-glass 本地登入**(`admin@uop.local`)。
  ...
  - 🟢 **F9-7(PATCH 四個 env 上 DEV)一早做咗** —— 2026-08-12 打 `/api/auth/sso/status` 返 `{"enabled":true}` 先發現(唔係新做,係發現咗做過)。🟢 **`F9-8` 嘅 break-glass 嗰半 2026-08-13 收咗**(`F6-6`:login 200 + `uop_access`/`uop_refresh` + role `ADMIN`)。🚧 **淨低 SSO 嗰半 —— 仍然零真人登入證據**:要喺 Entra 互動頁輸入公司帳號 + MFA ⇒ **AI 做唔到,要 Chris 本人撳一次**(30 秒;收貨 = 落到 `/` 而唔係彈返 login,兼 `/api/me` 返佢自己個 email)。⚠️ **呢個唔再係環境阻塞** —— 路通晒、掣着住、env 落齊,**淨係差一個人**
...
- **仍未做 / pending**:見 `BACKLOG.md`(🔴 **AUTH-2b 真 SSO e2e = W44 `F9-8` 嗰一撳**,唔再卡 IT app registration[ADR-0028 之後現有 app reg 直接用得],**卡緊 Chris 本人做一次 Entra 互動登入**;DEPLOY-harden;W43 遺留 = live close 未驗;🆕 🔴 **DEV 落後 `main` 五個 CH**[024/025/026/027/028,全部 08-12 merge,而 DEV 跑緊 08-10 嘅 `dev-86ed450`]⇒ 要部署 #6)。🟢 **CH-026 `G-7` 2026-08-13 做咗**(22 個 SKU curate 做 `unlimited`,`Available seats` `4,270,779` → `50,779`)。
