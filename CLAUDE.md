...
- ⚠️ **呢台機嘅 az session 唔穩定**(一日內撞過 **4 個唔同 SP**:`d2f094a3` / `a19dfe76` / `2ae44f00` / ACR 嗰個 `4a6e1474`)⇒ 錯身份會畀出**誤導性 error**。**做 az 操作之前一律先 `az account show` 驗身份**(2026-08-10 實測:`d2f094a3-…` = 部署 SP,sub `rcitest`)。🔴 **憑證唔喺 repo** —— 呢句原本寫住「喺 `apps/api/.env` 尾段」,2026-08-10 實測**唔啱**(`.env` 17 個 key 冇一個 `AZURE_*`;`aca.params.dev.json` 有 ACR 同 app secret 但冇部署 SP;`patch-deploy-dev.ps1` 自己唔 login,靠 caller 事先做)⇒ **要 Chris 喺 terminal 自己 `az login`**。ACR 側另一組憑證(`4a6e1474`)先至喺 `aca.params.dev.json`,而且**唔可以用 `az acr login`**(部署 SP 冇 registry 權限,CLI 會 fallback 互動式然後 `EOFError`,錯誤訊息完全唔提權限)。
