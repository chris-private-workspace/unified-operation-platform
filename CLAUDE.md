...
- **真正接得通企業網絡嘅環境 = `RG-RAPO-UOP-DEV`**(infra team 2026-08-04 交付 · 企業共用 ACA env + hub VNet PE + custom domain `rapo-uop-web-dev.rci-t.com`)—— **W44 進行中,未部署**。🔴 三個 blocker 全部要 infra team:**①企業中央 ACR 座標同 AcrPush 權**(給定值係 GUID,唔可能係 ACR 名)**②PG credential**(server admin 實測 `rcitadmin`,唔係畀嘅嗰個)**③ACA env 有冇 VNet 整合** —— ③ 唔止管 DB,**UOP → n8n outbound 一樣繫於佢**。見 ADR-0027(**Proposed**)· `docs/13-deployment/09-dev-as-built.md`。
