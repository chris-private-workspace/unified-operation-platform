...
- 🔴 **B9 = SSO 未接得到**(2026-08-06)。而家行緊 **break-glass 本地登入**(`admin@uop.local`)。infra 交嘅 app registration `08fa14bf-…`(tenant `d1ea071a-…`,**公司 M365 tenant**)**只配咗 client-credentials**,用戶登入三樣缺晒,全部有錯誤碼:①**冇任何 redirect URI** → `AADSTS900971`(唔係 50011 —— 係一條都冇)②**冇 Expose an API scope** → `AADSTS500011` ③**token 係 v1**(`ver: 1.0`)而 `jwt-auth.guard.ts:170-177` **精確比對** v2 issuer ⇒ **一定 401,而且登入會睇落成功**(「紅得靜」)。**2026-08-06 尾狀態**:①redirect URI 🟢 **infra 已補** ②Application ID URI / scope 🔴 **仍差**(`api://<client-id>` 唔存在,但 infra 可能設咗**另一個名** ⇒ 問「係咩」唔係叫佢「設定」)③token v1 🟢 **我哋自己解咗** —— `jwt-auth.guard.ts` 改成同時接受同一 tenant 嘅**兩個** issuer(`…/v2.0` + `sts.windows.net`),**`audience` 保持單一精確值**(放寬佢先至係真窿),879 test 全過,**唔開 ADR**(§5.1「唔屬架構改動」,但落手前有攞 Chris 拍板)。⇒ **SSO 淨係差一個答案:確切 Application ID URI。**⚠️ `VITE_ENTRA_*` 係 **build-time** ⇒ 估錯要重 build,所以問 infra 要**確切** Application ID URI。🟢 接 SSO **可回退**(`api.ts:25` local profile 優先 · `login.tsx` 本地表單永遠喺)。🟢 順帶:Graph app `27d329e5-…` 權限齊(`LicenseAssignment.ReadWrite.All` 等)⇒ F3-7 冇障礙。🔴 client secret **expiry 2028-07-28** 要入 RISK。
...
- 🟢 **但 B8 唔 block 驗證** —— 由**公司網絡**打 **ACA 預設 FQDN**(internal env 喺 hub VNet private DNS 一定有記錄):`https://aca-rapo-uop-web-dev.nicesea-c3849dba.eastasia.azurecontainerapps.io/` ⇒ F6-4/5/6 即刻收得,custom domain 嗰半留 B8 解封後補。
