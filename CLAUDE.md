...
- 🔴 **B7(新樽頸)= 觀測權限,唔係部署權限** —— 冇 `Microsoft.App/managedEnvironments/**read**` ⇒ `logs show` / `exec` 都 403;而 HTTP smoke 亦打唔到(`acaen-rapo-dev` 係 **internal-only env**,FQDN + `rci-t.com` 喺企業 DNS 同公網 DNS 都解析唔到,build host 喺 SGP VNet 唔喺嗰兩個網絡)。🔴 **加上 `apps/api/docker-entrypoint.sh` 明文令 migrate/seed 失敗 NON-FATAL ⇒ `Healthy` 證明唔到 DB 通** ⇒ **B3(ACA 連 private PG)· PG v18 migration · seed 三樣仍然係未知數,唔可以當已驗**。下一步三選(唔互斥):①infra 畀 `managedEnvironments/read` ②Chris 用個人帳號睇 Portal log ③由企業網絡內嘅機 curl。詳見 `docs/13-deployment/09-dev-as-built.md`。
