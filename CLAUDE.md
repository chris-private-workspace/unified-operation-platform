...
- 🔴 **B7(新樽頸)= 觀測權限,唔係部署權限** —— 冇 `Microsoft.App/managedEnvironments/**read**` ⇒ `logs show` / `exec` 都 403;而 HTTP smoke 亦打唔到(`acaen-rapo-dev` 係 **internal-only env**,FQDN + `rci-t.com` 喺企業 DNS 同公網 DNS 都解析唔到,build host 喺 SGP VNet 唔喺嗰兩個網絡)。🔴 **加上 `apps/api/docker-entrypoint.sh` 明文令 migrate/seed 失敗 NON-FATAL ⇒ `Healthy` 證明唔到 DB 通**。
- 🟢 **但 PG management plane metrics 補到大部分**(PG 喺我哋有 Contributor 嘅 RG ⇒ 讀得到,唔使 log/exec/企業網):`storage_used` 喺 api revision 起身嗰個窗口(`04:14:08Z`→`04:15`)跳 **+944 KB** 之後零變動 · `connections_failed` **全程 0** · `active_connections` **+2**。⇒ **B3(ACA 連 private endpoint PG)實質已證** · **PG v18 migration 強證據** · **seed 只證到「有寫入」,證唔到數目啱**。
