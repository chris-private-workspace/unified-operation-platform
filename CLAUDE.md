...
- 🔴 **W44 唯一硬 blocker = B1(image build)**。Registry = **`acrrci3ailanding1.azurecr.io`**(跨 tenant 企業中央 ACR)。`docker login` **通**(firewall 2026-08-05 修好),但 **`4a6e1474` / `d2f094a3` 兩個 SP 都冇 management plane** ⇒ `az acr build` 做唔到(🔴 **`AcrPush` 唔包 `scheduleRun/action`** —— 呢個係整件事嘅關鍵誤解);而本地 `docker build` 撞 **Docker Hub CDN 503**(⚠️ **MCR 通,淨係 Docker Hub 唔通**),registry 內 7 個 repo **冇 base image mirror**。⇒ 等 infra 三選一(①SP 攞 registry `read`+`scheduleRun/action` ②infra 代 build ③`az acr import` 兩個 base image)。**Chris 2026-08-05 拍板唔自建 ACR**(技術上已驗權限夠,但保持同 landing zone 設計一致)。詳見 `docs/13-deployment/09-dev-as-built.md`。
